...
print("Connects to the Addresses layer in IView2. It will take about 2-4 minutes. Please wait patiently")
url = "https://gisn.tel-aviv.gov.il/arcgis/rest/services/WM/IView2WM/MapServer/527/query"
...
while True:
    # Send a GET request to the ArcGIS REST API endpoint
    ...
    if 'exceededTransferLimit' in data:
        params['resultOffset'] = len(all_features)
    else:
        break

# Convert all features to a pandas DataFrame
dftable3 = pd.json_normalize(all_features)

# Create a mapping dictionary for column renaming
column_mapping = {col: col.replace('attributes.', '') for col in dftable3.columns}

# Rename the columns in the DataFrame
dftable2 = dftable3.rename(columns=column_mapping)

dftable = dftable2.sort_values(by='t_rechov')
...
def run ():
    ...
    shp_file = r"{}\Output.shp".format(savenamelist[0])
    shp_writer = shapefile.Writer(shp_file, shapeType=shapefile.POINT)

    # Define a dictionary to map Pandas data types to shapefile field types
    type_mapping = {
        'int64': 'N',
        'float64': 'F',
        'object': 'C'
    ...
    for field, dtype in zip(df.columns, df.dtypes):
        ...
        shp_writer.field(field, field_type)

    # Iterate over the rows of the DataFrame and add points to the shapefile
